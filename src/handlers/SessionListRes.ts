...
export class AgentSessionListResHandler implements MessageHandler<SessionListRes> {
  ...
  handle(msg: SessionListRes, ctx: HandlerContext): void {
    ...
    const items = msg.items.map((i) => ({
      ...
      visitorId: i.visitor_id,
      visitorNickname: i.visitor_nickname,
      visitorPhone: i.visitor_phone,
      visitorExternalId: i.visitor_external_id,
      agentId: i.agent_id,
      agentNickname: i.agent_nickname,
      status: i.status as SessionStatus,
      source: i.source,
      ip: i.ip,
      country: i.country,
      province: i.province,
      ...
      platform: i.platform,
      lastMsgContent: i.last_msg_content,
      lastMsgTime: i.last_msg_time,
