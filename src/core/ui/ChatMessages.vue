...
watch(() => props.messages.length, async (_, oldLen) => {
  await nextTick()
  if (!scrollRef.value) return
  if (oldLen === 0) { scrollRef.value.scrollTop = scrollRef.value.scrollHeight; return }
  if (isNearBottom.value) scrollRef.value.scrollTop = scrollRef.value.scrollHeight
})
